...
select count(*) as kolichestva_vseh_student from students;
...
select * from groups g inner join students s on s.group_id = g.id where g.id = 4;
--4. Сгруппируйте студентов по gender и выведите общее количество gender
select s.gender, count(s.gender) from students s inner join groups g on s.group_id = g.id group by s.gender;
--5. Найдите студента с айди 8 и обновите его данные
UPDATE students s  SET first_name = 'eldiyar', last_name = 'avazbekov',  date_of_birth = '2006-06-06',gender = 'Male' WHERE s.id = 8;
--6. Найдите самого старшего студента курса, айди курса которого равна 5
SELECT * FROM students s inner join courses c on s.id = c.groups_id where c.id = 5 ORDER BY s.date_of_birth ASC LIMIT 1;
...
select m.first_name, m.email, m.specialization from mentors m inner join groups g on m.course_id = g.id where g.group_name = 'Java 9';
...
select m.first_name from mentors m left join courses c on m.course_id = c.id where m.first_name like '';
--5. Вывести айди, имя ментора и его студентов
select m.id,m.first_name, s.first_name from mentors m inner join students s on m.course_id = s.group_id;
--6. Посчитать сколько студентов у каждого ментора, и вывести полное имя ментора и количество его студентов
SELECT COUNT(*) AS student_count, CONCAT(m.first_name, ' ', m.last_name) AS mentor_name FROM mentors m INNER JOIN students s ON m.course_id = s.group_id GROUP BY m.first_name, m.last_name;
--7. Вывести ментора у которого нет студентов
select m.first_name from mentors m left join students s on m.course_id = s.id where m.first_name like '';
--8. Вывести ментора у которого студентов больше чем 2
select * from mentors m inner join students s on m.email = s.email where m.experience > 2;
select m.first_name , m.last_name , count(s.id) from  mentors m inner join courses c on c.id = m.course_id inner join groups g on g.id = c.groups_id inner join students s on g.id = s.group_id group by m.first_name, m.last_name  having count(s.id) > 2;
--9. Вывести курсы ментора с айди 5
select * from mentors m inner join courses c on m.course_id = c.id where m.id =5;
--10.Вывести все уроки ментора, айди которого равен 5
select * from lessons l inner join mentors m on l.course_id = m.course_id where m.id = 5;
--------------LESSONS-----------------
--1. Вывести все уроки
select * from lessons;
--2. Получить все уроки студента, айди которого равен 2
select * from lessons l inner join students s on l.course_id = s.id where s.id =2;
--3. Посчитать уроки каждой группы и вывести название группы и количество уроков, где количество уроков больше чем 2
SELECT g.group_name, COUNT(l.id) AS lesson_count
FROM groups g
...
select * from students s inner join lessons l on s.id = l.course_id where s.id = 7 order by l.lesson_name ;
--5. Получить все уроки курса, где название курса 'Python kids'
select * from lessons l inner join courses c on l.course_id = c.id where c.course_name = 'Python';
